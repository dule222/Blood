        ...
        for (const donor of donors) {
            try {
                ...
                if (donor.tests && donor.tests.length > 0) {
                    ...
                    // Recalculate status for this donor
...
app.post('/api/webhook/deferral', async (req, res) => {
    ...
    try {
        ...
        // 🧠 Trigger counselling for deferral
...
async function recalculateDonorStatus(client, donor_id) {
    ...
    // Check if there are any reactive tests
...
async function checkAndCreateDeferral(client, donor_id, disease, result) {
    // Check if deferral already exists
    const existing = await client.query(`
    ...
    // Check if result requires deferral
    ...
    if (deferralTriggers[diseaseCode] && deferralTriggers[diseaseCode].includes(result)) {
        ...
        const retestDate = deferralType === 'Temporary' ? 
            new Date(Date.now() + 365 * 24 * 60 * 60 * 1000) : null; // 1 year later
...
async function triggerCounselling(client, donor_id, disease, result) {
    try {
        // Check if counselling already exists in last 30 days
        const existing = await client.query(`
        ...
        return { message: 'Counselling triggered successfully', session: result.rows[0] };
    } catch (err) {
...
// ─── START SERVER ───
app.listen(PORT, () => {
    console.log(`🩸 Multi-Disease Blood Bank API running on http://localhost:${PORT}`);
